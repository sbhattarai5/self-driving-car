...
class Sensor:
    ...
    def get_distance(self, audi, obj):
        ...
        if isinstance(obj, VehicleModel):
            return(math.sqrt(abs((pow(audi.x - obj.x, 2) +
                                  pow(audi.y - obj.y, 2)))))
        else:
            return(math.sqrt(abs((pow(audi.x - obj[0], 2) +
                                  pow(audi.y - obj[1], 2)))))
    def detectWorld(self,audi, WorldObjects):
        ...
        if audi.x < ROAD_X + (LANE_WIDTH):
            ...
            rect = viewwindow.transform_rect(ROAD_X, audi.y-(audi.h/PIXEL), 1/PIXEL, audi.h/PIXEL)
        ...
        if audi.x > ROAD_X + (LANE_WIDTH * (NUM_LANES - 1)):
            ...
            rect = viewwindow.transform_rect(ROAD_X + LANE_WIDTH * NUM_LANES, audi.y-(audi.h/PIXEL), 1/PIXEL, audi.h/PIXEL)
            pygame.draw.rect(surface, RED, rect, 1)
...
class VehicleModel:
    ...
    def __init__(self, image, x, y, dx, dy, ddx, ddy, target_dx, target_dy, max_dy,
                 angle, car_type, draw_bounding_box=False,
    ...
    def rotate(self):
        ...
        self.image = pygame.image.load('images/small/' + \
                                       self.car_type + '/' + \
                                       self.car_type + '-%s.png' % self.angle)
    def move(self):
        # change velocity
        ...
        if self.target_dy > self.dy: self.dy += self.ddy * delta_t
        if self.target_dy < self.dy: self.dy -= self.ddy * delta_t
...
class VehicleControlUser:
    ...
    def run(self):
        ...
        if keys[pygame.K_LEFT]:
            ...
            self.vehiclemodel.accn_depression = max(self.vehiclemodel.accn_depression + 0.1, 1.0)
